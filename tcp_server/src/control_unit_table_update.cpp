    ...
    if(master_commands.updated < updated_values.updated)
    {
        ...
        if( is_updated[IS_BASE_BUSY]==1 )
        {
            if( updated_values.is_base_busy == 0 || updated_values.is_base_busy==1 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"base",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[BASE_DESTINATION_X]==1 )
        {
            if( updated_values.base_destination_x>-2000.00 && updated_values.base_destination_x<2000.00 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"MySQL retrieval BASE_DESTINATION_X",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
        ...
        if( is_updated[BASE_DESTINATION_Y]==1 )
        {
            if( updated_values.base_destination_y>-2000.00 && updated_values.base_destination_y<2000.00 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"MySQL retrieval BASE_DESTINATION_Y",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if ( is_updated[BASE_FORWARD_STEP]==1 )
        {
            if( updated_values.base_forward_step<11 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"BASE_FORWARD_STEP",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[BASE_BACKWARD_STEP]==1 )
        {
            if( updated_values.base_backward_step<11 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"BASE_BACKWARD_STEP",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[BASE_ROTATE_LEFT]==1 )
        {
            if( updated_values.base_rotate_left<11 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"BASE_ROTATE_LEFT",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[BASE_ROTATE_RIGHT]==1 )
        {
            if( updated_values.base_rotate_right<11 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"BASE_ROTATE_RIGHT",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[IS_LEFT_ARM_BUSY]==1 )
        {
            if( updated_values.is_left_arm_busy >= 0 || updated_values.is_left_arm_busy <=100 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"IS_LEFT_ARM_BUSY",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[IS_RIGHT_ARM_BUSY]==1 )
        {
            if( updated_values.is_right_arm_busy >= 0 || updated_values.is_right_arm_busy <=100 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"IS_RIGHT_ARM_BUSY",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
        ...
        if( is_updated[LEFT_ARM_MOVE]==1 )
        {
            if( updated_values.left_arm_move >= 0 || updated_values.left_arm_move<=100 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"LEFT_ARM_MOVE",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[RIGHT_ARM_MOVE]==1 )
        {
            if( updated_values.right_arm_move >= 0 || updated_values.right_arm_move<=100 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"RIGHT_ARM_MOVE",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
        ...
        if( is_updated[EMOTION_STATUS]==1 )
        {
            if( updated_values.emotion_status<7 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"EMOTION_STATUS",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
        ...
        if( is_updated[LIGHT_STATUS]==1 )
        {
            if( updated_values.light_status > 6 && updated_values.light_status < 10 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"LIGHT_STATUS",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
        ...
        if( is_updated[PRINT_STATUS]==1 )
        {
            ...
            wizo_log(0,master_commands.updated,LOG_OK,"PRINT_STATUS",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);

            if(updated_values.print_status==2  )
            {
                //printf("                                                     print status is 2");
                //pthread_mutex_unlock(&mutex_printedData_update);
                pthread_mutex_lock(&mutex_printedData_update);
                checkin_status = "1";
                //printf("\n\nsignalling here------------------------------------------\n\n\n");
                pthread_cond_signal(&cond_printedData_update);
                pthread_mutex_unlock(&mutex_printedData_update);

            }
            else if(updated_values.print_status==5 )
            {
                //printf("                                                     print status is 2");
                //pthread_mutex_unlock(&mutex_printedData_update);
                pthread_mutex_lock(&mutex_printedData_update);
        ...
        if( is_updated[FACE_COGNITION_STATUS]==1 )
        {
            if( updated_values.face_cognition_status == 0 || updated_values.face_cognition_status==1 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"FACE_COGNITION_STATUS",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[CARD_READER_STATUS]==1 )
        {
            if( updated_values.card_reader_status == 0 || updated_values.card_reader_status==1 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"CARD_READER_STATUS",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[FACE_COGNITION_OUTCOME]==1 )
        {
            if( strpbrk(updated_values.face_cognition_outcome,key)==NULL )
            {
                ...
                //char buff[5];
                //sprintf(buff,"%i",updated_values.card_reader_status);
                wizo_log(0,master_commands.updated,LOG_OK,"FACE_COGNITION_OUTCOME",updated_values.face_cognition_outcome,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[ID_CARD_NAME]==1 )
        {
            if( strpbrk(updated_values.id_card_name,key)==NULL )
            {
                ...
                strcpy(master_commands.id_card_name, updated_values.id_card_name);
                    ...
                    wizo_log(0,master_commands.updated,LOG_OK,"ID_CARD_NAME",updated_values.id_card_name,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            ...
            else
            {
                ...
                wizo_log(0,master_commands.updated,LOG_ERROR,"ID_CARD_NAME",updated_values.id_card_name,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
        }
        if( is_updated[VISITEE_NAME]==1 )
        {
            if( strpbrk(updated_values.visitee_name,key)==NULL )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"MySQL retrieval VISITEE_NAME",updated_values.visitee_name,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[APPOINTMENT_DATE]==1 )
        {
            if( strpbrk(updated_values.appointment_date,key2)==NULL )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"MySQL retrieval APPOINTMENT_DATE",updated_values.appointment_date,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[APPOINTMENT_TIME]==1 )
        {
            if( strpbrk(updated_values.appointment_time,key2)==NULL )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"MySQL retrieval APPOINTMENT_TIME",updated_values.appointment_time,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[APPOINTMENT_LOCATION]==1 )
        {
            if( strpbrk(updated_values.appointment_location,key2)==NULL )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"MySQL retrieval APPOINTMENT_LOCATION",updated_values.appointment_location,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        {
            if( updated_values.speech_request >= 0)
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"SPEECH_REQUEST",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);

            }
            else
            {
                printf("\nWARNING: Field NOT updated. \nOut of range value dectected for speech_request field. Expected greater than 0. Received: %i\n\n",
                       updated_values.speech_request);
            }
        }
        if( is_updated[FACE_DETECT]==1 )
        {
            if( updated_values.face_detect >= 0 )
            {

                master_commands.face_detect = updated_values.face_detect;
                printf("face_detect value: %i\n",master_commands.face_detect);
//when face is detected then trigger to start listen, after listen perion speech status will be updated to 2 to stop listening. wait until ipad triggers back to 0 so listening can be triggered.
                if(updated_values.face_detect > 0 && master_commands.speech_status == 0 )
                {
                    master_commands.speech_status = LISTEN_START;//listen mode
                    epoch = 1;
                }
                else  if(updated_values.face_detect == 0 && master_commands.speech_status == 1 )
                {
                    master_commands.speech_status = 0;//listen stop mode
                    //epoch = 0;
                }


                char buff[10];
                sprintf(buff,"%i",updated_values.face_detect);
                wizo_log(0,master_commands.updated,LOG_OK,"FACE_DETECT",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);

            }
            else
            {
                printf("\nWARNING: Field NOT updated. \nOut of range value dectected for face_detect field. Expected between 0 and 1. Received: %i\n\n",
                       updated_values.face_detect);
            }
        }
        //********************************************************************
        if( is_updated[ARM_ACTIONS]==1 )
        {
            if( strpbrk(updated_values.arm_actions,key2)==NULL )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"ARM_ACTIONS",master_commands.arm_actions,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
        ...
        if( is_updated[CURRENT_LATITUDE]==1 )
        {
            if( updated_values.current_latitude > -90 || updated_values.current_latitude < 90 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"CURRENT_LATITUDE",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[CURRENT_LONGITUDE]==1 )
        {
            if( updated_values.current_longitude > -180 || updated_values.current_longitude < 180 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"CURRENT_LONGITUDE",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[CURRENT_ALTITUDE]==1 )
        {
            if( updated_values.current_altitude >  -180 || updated_values.current_altitude < 180 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"CURRENT_ALTITUDE",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[OBJ_FRAME]==1 )
        {
            if( strpbrk(updated_values.obj_frame,key2)==NULL )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"OBJ_FRAME",master_commands.obj_frame,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[OBJ_X]==1 )
        {
            if( updated_values.obj_x >  -1000 || updated_values.obj_x < 1000 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"OBJ_X",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[OBJ_Y]==1 )
        {
            if( updated_values.obj_y >  -1000 || updated_values.obj_y < 1000 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"OBJ_Y",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[OBJ_Z]==1 )
        {
            if( updated_values.obj_z >  -1000 || updated_values.obj_z < 1000 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"OBJ_Z",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[OBJ_HEIGHT]==1 )
        {
            if( updated_values.obj_height >  0 || updated_values.current_altitude < 1 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"OBJ_HEIGHT",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[OBJ_WIDTH]==1 )
        {
            if( updated_values.obj_width >  0 || updated_values.obj_width < 1 )
            {
                ...
                wizo_log(0,master_commands.updated,LOG_OK,"OBJ_WIDTH",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
            }
            else
            {
        ...
        if( is_updated[BATTERY_LEVEL]==1 )
        {
            if( updated_values.battery_level >= 0 && updated_values.battery_level <= 100)
            {
                master_commands.battery_level = updated_values.battery_level;
                printf("battery_level value: %i\n",master_commands.battery_level);

                char buff[5];
                sprintf(buff,"%i",updated_values.battery_level);
                wizo_log(0,master_commands.updated,LOG_OK,"BATTERY_LEVEL",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);

            }
            else
            {
                printf("\nWARNING: Field NOT updated. \nOut of range value dectected for battery_level field. Expected greater than 0. Received: %i\n\n",
                       updated_values.battery_level);
            }
        }
        if( is_updated[SPEECH_STATUS]==1 )
        {
            if( updated_values.speech_status >= 0 && updated_values.speech_status != 3)
            {
                master_commands.speech_status = updated_values.speech_status;
                printf("speech_status value: %i\n",master_commands.speech_status);
                if(updated_values.speech_status==3)
                epoch =1;
                char buff[5];
                sprintf(buff,"%i",updated_values.speech_status);
                wizo_log(0,master_commands.updated,LOG_OK,"SPEECH_STATUS",buff,master_commands.current_latitude,master_commands.current_longitude,master_commands.current_altitude);
